	...
	<div class="single">
		<div class="container">
			<div class="single-grids">				
					...
					<div class="flexslider">
						<ul class="slides">
							<li data-thumb="${pageContext.request.contextPath}/images/${theCake.img1}">
								<div class="thumb-image"> <a href="adminProducts.jsp"><img src="${pageContext.request.contextPath}/images/${theCake.img1}" data-imagezoom="true" class="img-responsive"></a> </div>
							</li>
				...
				<form action="${pageContext.request.contextPath}/cake/updateCake" method="post">			
				 <h6>名字：<input type="text" name="cakeName" value="${theCake.name }"/></h6>
				<h6>简介：<textarea rows="3" cols="19" name="infor" >${theCake.infor }</textarea></h6>
				<h6>用途：<select name="cakePurpose">
							<c:forEach var="bar" items="${bars }">
								<c:if test="${bar.barName == theCake.purpose}">
									<option value="${bar.barName }" selected="selected">${bar.barName }</option>
								</c:if>
								<c:if test="${bar.barName != theCake.purpose}">
				...
				<h6>类型：<select name="cakeType">
							<c:forEach var="type" items="${types }">
								<c:if test="${type.id == theCake.typeId}">
				...
				<h6>口味：<select name="cakeFlavour">
							<c:forEach var="flavour" items="${flavours }">
							<c:if test="${flavour.id == theCake.flavourId}">
							<option value="${flavour.id }" selected="selected">${flavour.flavourName }</option>
							</c:if>
							<c:if test="${flavour.id != theCake.flavourId}">
							<option value="${flavour.id }">${flavour.flavourName }</option>
							</c:if>
							</c:forEach>
						</select>
				</h6>
				<h6>颜色：<select name="cakeColor">
							<c:forEach var="color" items="${colors }">
							<c:if test="${color.color == theCake.color}">
				...
				<h6>层数：<input type="text" value="${theCake.floor }" name="cakeFloor">			
				</h6>
				<h6>折扣：<input type="text" value="${theCake.discounts }"name="cakeDiscount"/>
				</h6>
				</h6>
				<h6>最低价：<input type="text" value="${theCake.floorPrice }" name="cakeFloorPrice"/>
				</h6>
				<h6>图片1：<input type="file" name="img"/>
				</h6>
				<h6>图片2：<input type="file" name="img"/>
				</h6>
				<h6>图片3：<input type="file" name="img"/>
				</h6>	
				<div>
					<input type="submit" value="保存" class="add-cart item_add"/>
				</div>
				</form>
				</div>	
				<div class="col-md-4 single-grid1">
					<h2>价格</h2>
						<c:forEach var="size" items="${sizes }">
							<form action="${pageContext.request.contextPath}/size/addPrice" method="post">${size.size} 寸:
								<input type="text" name="price"  style="width:60px; height:30px;border: 1px solid #F07818;" value="${sc.price }"/>元
